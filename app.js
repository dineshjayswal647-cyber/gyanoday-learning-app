...
function isUserEnrolledInBatch(batchId) {
  if (state.user && state.user.role === 'admin') return true;
  return state.enrolledBatches && state.enrolledBatches.includes(batchId);
}
...
window.uploadProfilePhoto = function(event) {
  ...
  reader.onload = function(e) {
    ...
    img.onload = function() {
      ...
      // Get highly compressed small JPEG base64 string
      const compressedBase64 = canvas.toDataURL('image/jpeg', 0.7);

      if (state.user) {
        state.user.photo = compressedBase64;
        ...
        // Upload to server
        fetch(`${API_URL}/api/user/update-photo`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ phone: state.user.phone, photo: compressedBase64 })
        }).catch(err => console.warn("Offline, photo upload skipped.", err));
      }
    };
